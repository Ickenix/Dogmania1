...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'dog_vaccinations') THEN
    ...
    CREATE POLICY dog_vaccinations_delete_policy ON dog_vaccinations
      ...
      USING (dog_id IN (SELECT id FROM dogs WHERE owner_id = auth.uid()));
  ELSE
    -- If the table exists, check if policies exist and create them if they don't
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'dog_vaccinations' AND policyname = 'dog_vaccinations_select_policy') THEN
      CREATE POLICY dog_vaccinations_select_policy ON dog_vaccinations
        FOR SELECT TO public
        USING (dog_id IN (SELECT id FROM dogs WHERE owner_id = auth.uid()));
    END IF;

    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'dog_vaccinations' AND policyname = 'dog_vaccinations_insert_policy') THEN
      CREATE POLICY dog_vaccinations_insert_policy ON dog_vaccinations
        FOR INSERT TO public
        WITH CHECK (dog_id IN (SELECT id FROM dogs WHERE owner_id = auth.uid()));
    END IF;

    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'dog_vaccinations' AND policyname = 'dog_vaccinations_update_policy') THEN
      CREATE POLICY dog_vaccinations_update_policy ON dog_vaccinations
        FOR UPDATE TO public
        USING (dog_id IN (SELECT id FROM dogs WHERE owner_id = auth.uid()));
    END IF;

    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'dog_vaccinations' AND policyname = 'dog_vaccinations_delete_policy') THEN
      CREATE POLICY dog_vaccinations_delete_policy ON dog_vaccinations
        FOR DELETE TO public
        USING (dog_id IN (SELECT id FROM dogs WHERE owner_id = auth.uid()));
    END IF;
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'dog_medications') THEN
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'dog_health_records') THEN
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'dog_weight_history') THEN
