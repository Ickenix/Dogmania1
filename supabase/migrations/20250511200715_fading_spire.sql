...
-- Create policies for stripe_customers (only if they don't exist)
DO $$
BEGIN
  IF NOT EXISTS (
...
-- Create policies for stripe_subscriptions (only if they don't exist)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
...
-- Create policies for stripe_orders (only if they don't exist)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
