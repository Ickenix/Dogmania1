/*
  ...
  1. New Tables
    ...
    - `dog_weight_history` - Stores weight history for dogs
  
  2. Security
    - Enable RLS on all tables
    - Add policies for authenticated users to manage their own dogs' health data
*/

-- Create dog_vaccinations table if it doesn't exist
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'dog_vaccinations') THEN
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'dog_medications') THEN
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'dog_health_records') THEN
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'dog_weight_history') THEN
    CREATE TABLE dog_weight_history (
      id uuid PRIMARY KEY DEFAULT uuid_generate_v4(),
      dog_id uuid NOT NULL,
